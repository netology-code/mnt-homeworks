...
2. Перейдите в каталог с ролью vector-role и создайте сценарий тестирования по умолчаню при помощи `molecule init scenario --driver-name docker`.
...
1. Добавьте в директорию с ansible-clickhouse ролью файл tox.ini из [директории](./example)
2. Запустите `docker run --privileged=True -v <path_to_repo>:/opt/ansible-clickhoyse -w /opt/ansible-clichouse -it <image_name> /bin/bash`, где path_to_repo - путь до корня репозитория с ansible-clickhouse на вашей файловой системе.
